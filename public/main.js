...
socket.onmessage = (event) => {
  ...
  const strength = data.field_strength;
  document.getElementById("b_strength").textContent =
    strength.toFixed(2) + " µT";
  ...
  // 4. Drive aurora intensity from real field strength
  auroraMaterial.uniforms.intensity.value =
    THREE.MathUtils.clamp(strength / 60, 0.1, 1.0);

  // 5. Feed the graph
  updateGraph(strength);

  // 6. Auto-trigger solar storm if field is very high
  if (strength > 55 && !sunWaveLocked) {
    sunWave.click();
  }
};
...
// Handle commands FROM website → ESP32
